...
let user = {
  ...
  sayHi() {
	...
	/=/ Мы можем взять этот метод и приложить к другому объекту
};

user.sayHi(); /=/ John
/=/ Метод использует `name`(this.name) относительно того обьекта к которому был вызван
```
...
## _fluent interface - цепочка методов_

Основная идея` fluent interface` заключается в том, чтобы создать последовательность вызовов `методов`, которые можно связать вместе, чтобы получить читаемый и "потоковый" код.

### _return `this`_

`return this` позволяет комбинировать методы, потому что `возвращает` текущий `объект`,
на котором `метод` был `вызван`.
...
const name = person.setName("Jane").getName();
```

Когда вы вызываете `метод` на объекте и возвращаете `this`, вы фактически возвращаете сам `объект`, что позволяет вызывать другие методы на этом же `объекте`.

- Возвращая `this` в методах, мы продолжаем работать одним и тем же `объектом`.
- Следовательно, каждый метод в цепочке `изменяет` тот же самый `объект`.
- Более того, вероятно этот `объект` уже был `изменен` предыдущими `методами` в цепочке.
...
class Calc {
  ...
  add(arg) {
    const newVal = this.value + arg;
    return new Calc(newVal);
  }
...
Когда `функция` присваивается `переменной`, она сохраняет только `ссылку` на эту `функцию`, но **_не сохраняет_** `исходный контекст` вызова.
Тк мы вызываем функцию `boundFn()` без указания нужного нам контекста(без элемента перед точкой), то контекст ссылается не на `user`, а на глобальный `this` - объект `windows`.

*`Исходный контекст` вызова включает в себя значение `this` и любые `аргументы`, переданные при вызове функции.*
- `параметры` функции
