**Замыкание** — это функция, которая запоминает и имеет доступ к переменным из доступной ей области видимости, даже после того, как эти переменные перестали существовать. (когда родительская функция завершена)
