...
2. Блоку не следует задавать внешнюю геометрию.
   - У блока не должно быть отступов, границ, влияющих на размеры и позиционирования. Это позволит свободно переносить переиспользуемый элемент в любое окружение
3. Для селекторов необходимо использовать `.class`, и не использовать селекторы по тегам или id
...
<div class="about">
    ...
    * К Блокам Title, Subtitle добавлены кастомные стили Элементов 
...
### *Пример записи стилей для БЭМ-HTML*
