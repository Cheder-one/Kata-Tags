...
- `POST` - не идемпотентен, тк создает 4 разных ресурса
  ![[Pasted image 20231115224612.png]]

- `PUT`(как и корректные методы `GET, PUT, DELETE`) - `идемпотентен`. Тк при дублировании запроса ничего нового не произойдет. Произойдет обновление одного и того же атрибута.
